...
export default function SectionDetail() {
  ...
  async function handleRegister() {
    ...
    try {
      ...
      if (res.status === 'enrolled') {
        ...
        setResult({ type: 'success', message: `Added to waitlist at position ${(res as any).position}.` })
      } else {
        const code = (res as any).error_code ?? ''
        setResult({ type: 'error', message: ERROR_MESSAGES[code] ?? (res as any).message ?? 'Registration failed.' })
      }
    } catch (err: any) {
  ...
  async function handleJoinWaitlist() {
    ...
    try {
      ...
      setResult({ type: 'error', message: err.response?.data?.detail ?? 'Could not join waitlist.' })
    } finally {
      setActionLoading(false)
    }
  }
