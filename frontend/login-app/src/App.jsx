...
function storeStudentAuth(result) {
  const token = result?.access_token
  if (!token) return
  localStorage.setItem('crsp_token', token)
  localStorage.setItem('crspStudentToken', token)
  if (result.student_number) {
    localStorage.setItem('crsp_user_id', String(result.student_number))
  }
}

function storeProfessorAuth(result) {
  const token = result?.access_token
  if (!token) return
  localStorage.setItem('crspProfessorToken', token)
}

function storeAdminAuth(result) {
  const token = result?.access_token
  if (!token) return
  localStorage.setItem('crspAdminToken', token)
}
...
function Field({ id, label, name, type = 'text', placeholder, autoComplete }) {
  return (
    <div className={styles.field}>
      <label htmlFor={id}>{label}</label>
      <input id={id} name={name} type={type} placeholder={placeholder} autoComplete={autoComplete} required />
    </div>
  )
}
...
// ─────────────────────────────────────────────────────────────────────────────
function useFormSubmit(endpoint, buildPayload, options = {}) {
...
function ManualStartForm() {
  const { loading, status, handleSubmit } = useFormSubmit(
    ...
    }),
    {
      onSuccess: storeStudentAuth,
      redirectTo: result =>
        result?.requires_profile_completion ? '/student/profile/manual' : '/dashboard',
...
export default function App() {
  ...
  return (
    <div className={`${styles.root} login-app-root`}>
      <Header />
        ...
        <div className={styles.card}>
          ...
          <section className={styles.formPanel}>
            <h1 className={styles.portalTitle}>
              <span className={styles.iutBold}>CR&amp;S</span>
              <span className={styles.portalRest}>&nbsp;System</span>
            </h1>
